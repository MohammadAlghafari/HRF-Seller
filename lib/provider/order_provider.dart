...
class OrderProvider extends ChangeNotifier {
  ...
  Future<ResponseModel> updateOrderPaidStatus(int id,) async {
    _isLoading = true;
    notifyListeners();

    ResponseModel responseModel;
    ApiResponse apiResponse;
    apiResponse = await orderListRepo.orderPaidStatus(id,);
    ...
    if (apiResponse.response != null && apiResponse.response.statusCode == 200) {
      ...
      if(_status != null) {
        responseModel = ResponseModel(false, apiResponse.response.data['message']);
        _addOrderStatusErrorText = apiResponse.response.data['message'];
         _orderList.firstWhere((element) => element.id == id).paymentStatus = 'paid';
      }else {
        responseModel = ResponseModel(true, message);
        _addOrderStatusErrorText = message;
        _orderList.firstWhere((element) => element.id == id).paymentStatus = 'paid';
